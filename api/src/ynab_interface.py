...
class YNABInterface:
    ...
    def get_all_transactions(self, start_date=None, end_date=None, filter_irrelevant=False):
        ...
        filtered_transactions = [{k: v for k, v in txn.items() if k in ['date', 'amount', 'payee_name', 'category_name']} for txn in all_transactions]
        ...
        if filter_irrelevant:
           df = df[~df['payee_name'].str.contains('transfer|chase credit crd epay|starting balance', case=False, na=False)]
    ...
    def get_account_id_list(self):
        accounts_url = f"https://api.youneedabudget.com/v1/budgets/{self._budget_id}/accounts"
        response = requests.get(accounts_url, headers=self._headers)
        accounts_list = response.json().get('data').get('accounts')
        account_ids = [account.get('id') for account in accounts_list if not account.get('closed', True)]
...
if __name__ == '__main__':
    ...
    ynab_interface.get_all_transactions(start_date='2023-07-01', end_date='2023-07-31', filter_irrelevant=True)
